...
def create_movie_table():
    c.execute('CREATE TABLE IF NOT EXISTS movie(name VARCHAR(60) not null, '
              'budget INTEGER, year INTEGER, Duration TIME, Rating VARCHAR(20), Genre VARCHAR(100), Id INTEGER UNIQUE NOT NULL,language VARCHAR(25))')

def create_person_table():
     c.execute('CREATE TABLE IF NOT EXISTS person(name CHAR(30), birthday DATE, id INTEGER UNIQUE)')
...
def viewMoviesAlpha():
    c.execute('SELECT * FROM movie ORDER BY name')
    data = c.description
    d = "";
    for i in data:
        d = d + i[0] +"\t"
...
def findMoviesBy(person):
    command = "SELECT M.name,M.year,M.rating,M.Genre,M.language FROM movie M, Crew C, person P "
...
def selectYear(year):
    c.execute('SELECT * FROM movie WHERE year = ?', (year,))
...
def findGenre(genre):
    command = "SELECT * FROM movie WHERE Genre LIKE \"%"+ genre + "%\""
    c.execute(command)
    data = c.description
    d = "";
    for i in data:
